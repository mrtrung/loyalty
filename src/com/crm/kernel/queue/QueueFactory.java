...
public class QueueFactory {
	...
	public static QueueSession createQueueSession(QueueConnection connection)
			throws Exception {
	...
	public static Queue getQueue(String queueName) throws Exception {
		...
		/*
		 * try { queue = appQueues.get(queueName);
		 * 
		 * if (queue == null) { if (context == null) initContext(); queue =
		 * (Queue) context.lookup(queueName);
		 * 
		 * appQueues.put(queueName, queue); } } catch
		 * (javax.naming.NamingException e) { throw new
		 * AppException("queue-not-found", queueName); } catch (Exception e) {
		 * throw e; }
		 */
		...
		synchronized (mutex) {
			...
			try {
				if (context == null) {
					...
					DAILY_RENEW = configProvider.getString("queue.dailyRenew",
							"queue/DailyRenew");
