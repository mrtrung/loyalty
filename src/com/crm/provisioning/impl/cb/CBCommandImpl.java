		...
		else
		{
			try
			{
				...
				try
				{
					...
					instance.debugMonitor("isdn: " + isdn + " - cbstatus: " + cbStatus);
					...
					if (cbStatus == 2 || cbStatus == 3)
					{
						...
						if (!result.isPostpaid() && createPrepaidInfor(result,"createSubscriber"))
						{
							...
							if (baseProductInstanceId < 0)
							{
								...
								connection.createPrepaidSubscriber(isdn, result, respProperties);							
							}
						}					
			...
			throws Exception
	{
		...
		else
		{
			...
			try
			{
				...
				boolean retryRenewal = product.getParameter("RetryRenewal", "false").equals("true");
				int maxRetryTime = Integer.parseInt(product.getParameter("MaxRetryTime", "5"));
				long retryWaitingTime = Long.parseLong(product.getParameter("RetryWaitingTime", "5"));
				...
				{
					cbStatus = connection.getServiceStatus(schema, serviceTypeId,productId, isdn, respProperties);
					instance.debugMonitor("isdn: " + isdn + " - cbStatus before renewal: " + cbStatus);
					...
					else
					{
						...
						{
							retryTime++;
							instance.debugMonitor("Retry " + retryTime + " times isdn: " + isdn + " - cbstatus: " + cbStatus);
				...
				if (cbStatusConvert == 2 || cbStatusConvert == 3)
				{
					...
					String strCommand = "REGISTER(" + isdn + "), CB_STATUS=" + cbStatusConvert + ", PRODUCTID=" + productId + ";";
					long sessionId = setRequest(instance, result, strCommand);
		...
		else
		{
			try
			{
				...
				try
				{
					...
					cbStatus = connection.getServiceStatus(schema, serviceTypeId,
							productId, isdn, respProperties);
					instance.debugMonitor("isdn: " + isdn + " - cbStatus after unregister: " + cbStatus);
					
					ProductEntry product = ProductFactory.getCache().getProduct(result.getProductId());
					if (product.getParameter("PendingRegisterVB", "false").equals("true"))
					{
								...
								&& !result.getActionType().equals(Constants.ACTION_UNREGISTER))
						{
							...
							Thread.sleep(Long.parseLong(product.getParameter("TimePending", "10000")));
						}
					}
				}
				finally
			...
			throws Exception
	{
		...
		else
		{
			try
			{
				...
				String isdn = transformISDN(result.getIsdn());
				// check subscriber is existed or not
				Long productId = Long.parseLong(provisioningCommand.getParameter("PostpaidProductId", ""));
				Long serviceTypeId = CBConnection.POSTPAID_SERVICE_TYPE_ID;
				if (result.isPrepaid())
					serviceTypeId = CBConnection.PREPAID_SERVICE_TYPE_ID;
				...
				try
				{
					...
					String productAlias = provisioningCommand.getParameter("ProductAlias", "");
					ProductEntry product = ProductFactory.getCache().getProduct(productAlias);
					SubscriberProduct subscriberProduct = SubscriberProductImpl
					...
					if (product.getParameter("PendingRegister", "false").equals("true"))
					{
						if (!result.getActionType().equals(Constants.ACTION_CANCEL)
								&& !result.getActionType().equals(Constants.ACTION_UNREGISTER))
						{
			...
			throws Exception
	{
		...
		else
		{
			try
			{
				...
				String strCommand = "checkTimeUsed(" + result.getIsdn() + "), PRODUCTID=" + result.getProductId();
