...
//			return 0;
			...
			throws Exception
	{
		...
		try
		{
			...
			updateOrder(connection, orderId, orderDate, orderType, subscriberId, isdn, subscriberType, subProductId, productId, price,
			...
			throws Exception
	{
		...
		try
		{
			...
			stmtOrder.setString(5, isdn);
			stmtOrder.setInt(6, subscriberType);
			stmtOrder.setDouble(7, price);
			stmtOrder.setDouble(8, quantity);
			stmtOrder.setDouble(9, discount);
			stmtOrder.setDouble(10, amount);
			stmtOrder.setDouble(11, score);
			stmtOrder.setInt(12, status);
			stmtOrder.setString(13, cause);
			stmtOrder.setString(14, channel);
			stmtOrder.setTimestamp(15, DateUtil.getTimestampSQL(orderDate));
			stmtOrder.setTimestamp(16, DateUtil.getTimestampSQL(orderDate));
			stmtOrder.setLong(17, orderId);
	...
	public static void updateStatus(
			Connection connection, long orderId, Date orderDate, int status, String cause, String description, long subProductId, long campaignId)
			throws Exception
	{
		PreparedStatement stmtOrder = null;
		...
		{
			String sql = "Update SubscriberOrder "
					+ "   Set 	modifiedDate = sysDate, status = ?, cause = ?, description = ?, subproductid = ?, campaignId = ? "
					+ "Where orderDate >= trunc(?) and orderDate < (trunc(?) + 1) and orderId = ? ";
			...
			stmtOrder.setTimestamp(6, DateUtil.getTimestampSQL(orderDate));
			stmtOrder.setTimestamp(7, DateUtil.getTimestampSQL(orderDate));
			stmtOrder.setLong(8, orderId);
	...
	public static void updateStatus(long orderId, Date orderDate, int status, String cause, String description, long subProductId, long campaignId) throws Exception
	{
		Connection connection = null;
...
//			Thread.sleep(sleepTime);
			...
			updateStatus(connection, orderId, orderDate, status, errorCode, description, subProductId, campaignId);
		}
		catch (Exception e)
		{
			throw e;
	...
	public static double getChargingMoney(String isdn, String rechargeTable,
			Date startDate) {
	...
	public static void updateStatus(long orderId, Date orderDate, int status,
			String cause, String description) {
		// TODO Auto-generated method stub
